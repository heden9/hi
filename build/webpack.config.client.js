...
const config = webpackMerge(baseConfig, {
  entry: {
    app: path.join(__dirname, '../src/index.js'), // path.join是node的语法，用于标志当前项目的根目录
    vendor: ['react', 'react-dom', 'react-router', 'antd-mobile', 'dva', 'prop-types'],
  },
  output: {
    filename: '[name].[hash].js',
    path: path.join(__dirname, '../dist'),
    publicPath: '/', // 用于打包到html时，直接加在静态资源的前面。线上可写cdn路径
  },
  plugins: [
